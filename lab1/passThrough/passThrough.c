...
--*/
{
    ...
    UNICODE_STRING required_extension = RTL_CONSTANT_STRING(L"ulyana"); // Перевод расширения в UNICODE_STRING

    if (!NT_SUCCESS(status)) { //выдает статус true, если файл открыт. Проверка открыт ли файл
        //ничего не выводим, чтобы не загружать оперативную память, DebugView
    }
    else if ( // проверка на совпадение расширения с нашим шаблоном
        RtlEqualUnicodeString(
        ...
        if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) {
            ...
            if (Data->Iopb->Parameters.Write.WriteBuffer) {//проверяем, что в буфере что-то есть
                ...
                unsigned char KEY[] = "86364715A2BD267F";//определение ключа
